			...
			<table class="table table-hover table-bordered">
				...
				<tbody>
					<c:forEach var="question" items="${ questionList }">
						<tr>
						   	<td><a href="updateQuestion.do?seq=${question.getSeq()}"
								style="text-decoration: none">${ question.seq }</a></td>
		...
		<ul class="col pagination justify-content-center">
			...
			<c:if test="${ fp != 1 }">
				<li class="page-item"><a
					href="getQuestionList.do?curPage=1&rowSizePerPage=${rp}&searchType=${st}&searchWord=${sw}"
					class="page-link"><i class="fas fa-fast-backward"></i></a></li>
				<li class="page-item"><a
					href="getQuestionList.do?curPage=${fp-1}&rowSizePerPage=${rp}&searchType=${st}&searchWord=${sw}"
					class="page-link"><i class="fas fa-backward"></i></a></li>
			</c:if>

			<c:forEach var="page" begin="${fp}" end="${lp}">
				<li class="page-item ${cp==page ? 'active' : ''}"><a
					href="getQuestionList.do?curPage=${page}&rowSizePerPage=${rp}&searchType=${st}&searchWord=${sw}"
					class="page-link">${page}</a></li>
			</c:forEach>

			<c:if test="${ lp < tp }">
				<li class="page-item"><a
					href="getQuestionList.do?curPage=${lp+ps}&rowSizePerPage=${rp}&searchType=${st}&searchWord=${sw}"
					class="page-link"><i class="fas fa-forward"></i></a></li>
				<li class="page-item"><a
					href="getQuestionList.do?curPage=${tp}&rowSizePerPage=${rp}&searchType=${st}&searchWord=${sw}"
