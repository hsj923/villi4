...
//   제목,작성자,카테고리로 검색하기
   ...
   private String selectBoardListByAddress = "";
   ...
   public void getSqlPropeties() {
      ...
      selectBoardListByAddress= environment.getProperty("selectBoardListByAddress");
   }
   ...
   public int getTotalRowCount(SearchVO searchVO) {
      ...
      if(searchVO.getSearchType()==null || searchVO.getSearchType().isEmpty() ||
         ...
         searchVO.setSearchType("address");
      } else {         
         if(searchVO.getSearchType().equalsIgnoreCase("address")) {
            sql = boardTotalRowCount + " and address like '%" + searchVO.getSearchWord() + "%'";
         ...
         }   else if(searchVO.getSearchType().equalsIgnoreCase("title")) {
   ...
   public BoardVO insertBoard(BoardVO board) {
      jdbcTemplate.update(insertBoard, board.getTitle(), board.getAddress(), board.getNickname(), board.getContent(), board.getCate2(), board.getPrice(), board.getUsedate(), board.getDuedate(), board.getFileName1(), board.getFileName2(), board.getFileName3());
      return board;
   }   
