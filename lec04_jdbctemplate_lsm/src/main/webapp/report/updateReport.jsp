...
<body>
							...
							<div class="collapse navbar-collapse" id="navbarSupportedContent">
								<ul class="navbar-nav me-auto mb-2 mb-lg-0">
									<li class="nav-item"><a class="nav-link active"
										aria-current="page" href="report/insertReport.jsp">글작성</a></li>
			...
			<div class="card">
				<div class="card-body">
					<h5 class="card-title text-start">
						<img src="resources/images/noimg.png"
							...
							height="75"><span>${report.writer }</span> <span
				...
				<ul class="list-group list-group-flush">
					<li class="list-group-item text-start"><c:choose>
							<c:when test="${report.status eq '모집중'}">
								<span class="badge bg-success text-white rounded-pill ">${report.status}</span>
							</c:when>
							<c:when test="${report.status eq '모임종료'}">
								<span class="badge bg-danger text-white rounded-pill ">${report.status}</span>
							</c:when>
						</c:choose> <span class="fs-4 fw-bold">${ report.title }</span>
						<p class="fs-6 fst-italic">${report.regDate}</p> <svg
...
</svg><span class="fs-6 ms-2">${ report.place }</span><br> <svg
							xmlns="http://www.w3.org/2000/svg" width="16" height="16"
...
</svg><span class="fs-6 ms-2">${ report.per }명 모집</span><br> <svg
							xmlns="http://www.w3.org/2000/svg" width="16" height="16"
  ...
  <path
...
</svg><span class="text-muted fs-6 ms-2">${ report.meet_date }</span> <br>
					<br>


						<p class="text-muted fs-5">${ report.content }</p> <br> <br> <br>
						<p class="mt-4">조회 : ${ report.cnt }</p></li>
		...
		<div class="container row-3" align="center">
			...
			<a href="deleteReport.do?seq=${report.getSeq()}"
				class="btn btn-dark my-5 mx-2">게시글삭제</a> <a href="getReportList.do"
