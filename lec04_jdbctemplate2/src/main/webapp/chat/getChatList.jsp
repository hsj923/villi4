...
<body>
			...
			<input type="hidden" id="rowSizePerPage" name="rowSizePerPage" value="${searchVO.getRowSizePerPage()}">
	   		<div class="row mt-3 justify-content-center">
				...
				<div class="col-5 btn-group"> 		
			    	...
			    	<a href="chat/insertChat.jsp" class="col-1 btn btn-primary me-2">사용자등록</a>
 ...
 -->
					...
					<c:forEach  var="chat" items="${ chatList }">
						<tr>
							<td scope="row"><a href="updateChat.do?id=${ chat.getWriter() }">${ chat.getWriter() }</a></td>
							<%-- <td>${ chat.getTitle() }</td>
							<td>${ chat.getWriter() }</td> --%>
							<td align="center">
								<a href="deleteChat.do?id=${ chat.getWriter() }" class="btn btn-danger"><i class="fas fa-trash"></i>
								</a>
							</td>
						</tr>
					</c:forEach>				
	...
	<script>
		$(function() {
			...
			$('#btnReset').click(function() {
				$('#curPage').val(1);
				$('#chatForm').find("select[name='searchCategory'] option:eq(0)").attr("selected", "selected");
				$('#chatForm').find("select[name='searchType'] option:eq(0)").attr("selected", "selected");
				$('#chatForm').find("input[name='searchWord']").val("");
				$('#chatForm').submit();
			}); // #id_btn_reset.cli			
