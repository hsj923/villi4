...
public class VoteRowMapper implements RowMapper<VoteVO> {
	public VoteVO mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		vote.setFileName1(rs.getString("FILENAME1"));
		vote.setFileName2(rs.getString("FILENAME2"));
		vote.setFileName3(rs.getString("FILENAME3"));
		vote.setFileName3(rs.getString("FILENAME4"));
		return vote;
	}
}